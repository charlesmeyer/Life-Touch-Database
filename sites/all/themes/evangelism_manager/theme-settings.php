...
function evangelism_manager_settings($saved_settings, $subtheme_defaults = array()) {

  // Get the default values from the .info file.
  $defaults = evangelism_manager_theme_get_default_settings('evangelism_manager');
  ...
  $form['evangelism_manager_zen_tabs'] = array(
    '#type'          => 'checkbox',
    '#title'         => t('Use Zen Tabs'),
    '#default_value' => $settings['evangelism_manager_zen_tabs'],
    '#description'   => t('Replace the default tabs by the Zen Tabs.'),
    '#prefix'        => '<strong>' . t('Zen Tabs:') . '</strong>',
  );

  $form['evangelism_manager_block_editing'] = array(
    '#type'          => 'checkbox',
    ...
    '#default_value' => $settings['evangelism_manager_block_editing'],
    '#prefix'        => '<strong>' . t('Block Edit Links:') . '</strong>',
  );

  $form['themedev']['evangelism_manager_rebuild_registry'] = array(
    '#type'          => 'checkbox',
    '#title'         => t('Rebuild theme registry on every page.'),
    '#default_value' => $settings['evangelism_manager_rebuild_registry'],
    '#description'   => t('During theme development, it can be very useful to continuously <a href="!link">rebuild the theme registry</a>. WARNING: this is a huge performance penalty and must be turned off on production websites.', array('!link' => 'http://drupal.org/node/173880#theme-registry')),
    '#prefix'        => '<div id="div-evangelism_manager-registry"><strong>' . t('Theme registry:') . '</strong>',
    '#suffix'        => '</div>',
  );
...
function _evangelism_manager_theme(&$existing, $type, $theme, $path) {
  ...
  evangelism_manager_theme_get_default_settings($theme);
...
function evangelism_manager_theme_get_default_settings($theme) {
  $themes = list_themes();
