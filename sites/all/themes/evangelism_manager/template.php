...
if (theme_get_setting('evangelism_manager_rebuild_registry')) {
  drupal_rebuild_theme_registry();
}

// Add Zen Tabs styles
if (theme_get_setting('evangelism_manager_zen_tabs')) {
  drupal_add_css( drupal_get_path('theme', 'evangelism_manager') .'/css/tabs.css', 'theme', 'screen');
}
...
function evangelism_manager_preprocess_page(&$vars, $hook) {
  ...
  if (!$vars['is_front']) {
    ...
    $body_classes[] = evangelism_manager_id_safe('page-'. $path);
    $body_classes[] = evangelism_manager_id_safe('section-'. $section);
...
function evangelism_manager_preprocess_node(&$vars, $hook) {
  ...
  if ($vars['type'] == "prospect") {
    $prospect_type = "";
    if ($vars['node']->field_saved[0]['value'] == '0') {
      $prospect_type = "salvation";	
    } 
    elseif ($vars['node']->field_baptized[0]['value'] == '0') {
      $prospect_type = "baptism";
    }
    elseif ($vars['node']->field_attended[0]['value'] == '0') {
      $prospect_type = "attendance";
    }
    elseif ($vars['node']->field_sundayschool[0]['value'] == '0') {
      $prospect_type = "sundayschool";
    }
    elseif ($vars['node']->field_member[0]['value'] == '0') {
      $prospect_type = "membership";
    }
    else {
      $prospect_type = "discipleship";
    }
    $classes[] = $prospect_type;
  }
  
  // check to see if prospect as been contacted in the past week
  if (time() - $vars['node']->field_last_contact[0]['value'] > 604800) {
    $classes[] = 'nocontact';
  }
  
  if ($vars['node']->field_assigned[0]['value'] == '1') {
      $classes[] = "assigned";	
   }
  
  // Class for node type: "node-type-page", "node-type-story", "node-type-my-custom-type", etc.
  $classes[] = evangelism_manager_id_safe('node-type-' . $vars['type']);
...
function evangelism_manager_preprocess_block(&$vars, $hook) {
    $block = $vars['block'];

    // special block classes
    $classes = array('block');
    $classes[] = evangelism_manager_id_safe('block-' . $vars['block']->module);
    $classes[] = evangelism_manager_id_safe('block-' . $vars['block']->region);
    $classes[] = evangelism_manager_id_safe('block-id-' . $vars['block']->bid);
    $classes[] = 'clearfix';
    
    
    $vars['block_classes'] = implode(' ', $classes); // Concatenate with spaces

    if (theme_get_setting('evangelism_manager_block_editing') && user_access('administer blocks')) {
...
function evangelism_manager_menu_item_link($link) {
...
function evangelism_manager_menu_local_tasks() {
  $output = '';
...
function evangelism_manager_menu_item($link, $has_children, $menu = '', $in_active_trail = FALSE, $extra_class = NULL) {
...
#New line added to get unique classes for each menu item
  $css_class = evangelism_manager_id_safe(str_replace(' ', '_', strip_tags($link)));
...
function evangelism_manager_id_safe($string) {
...
function evangelism_manager_breadcrumb($breadcrumb) {
  if (!empty($breadcrumb)) {
